    ...
    fs.exists(cache_path, function(is_cached) {
      ...
      if (is_cached) {
        ...
        http_request_movie(id, function(err, data) {
          data['custom'] = {}
          data['custom']['boxoffice_text'] = rev

          var sum = parseFloat(rev.substr(0, rev.length - 1))
          if (rev.slice(-1) === 'M') sum *= 1e6
          if (rev.slice(-1) === 'K') sum *= 1e4
          data['custom']['boxoffice_num'] = sum
